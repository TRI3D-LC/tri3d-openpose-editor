...
import controlnet_aux.open_pose.util
from controlnet_aux.util import (HWC3, resize_image)

from controlnet_aux.open_pose.body import (Body, BodyResult, Keypoint)
...
def draw_poses(poses: List[PoseResult],
               H,
               W,
               draw_body=True,
               draw_hand=True,
...
def do_inference(input_image, output_file_prefix, width=512, height=512):
    output_json_file = output_file_prefix + '.json'
    output_txt_file = output_file_prefix + '.txt'

    model_pose = OpenposeDetector.from_pretrained("lllyasviel/ControlNet")
    image = load_image(input_image)
    image_pose = model_pose(image)

    def process_points(x, y, inwidth, inheight):
        if x < 0:
            x = int(-1)
        else:
            x = int(inwidth * x)
    ...
    def process_points_2(inpts):
        return process_points(x=inpts[0],
                              y=inpts[1],
                              inwidth=width,
                              inheight=height)

    keypoints = [process_points_2(i) for i in image_pose[1]]
    ...
    outdata = json.dumps(ret)
    with open(output_json_file, 'w', encoding='utf-8') as f:
        f.write(outdata)

    ret['txt_file'] = output_txt_file
    ret['json_file'] = output_json_file
    return ret
...
class OpenposeDetector:
    ...
    def from_pretrained(cls,
        ...
        if os.path.isdir(pretrained_model_or_path):
            ...
            hand_model_path = os.path.join(pretrained_model_or_path,
                                           hand_filename)
            face_model_path = os.path.join(face_pretrained_model_or_path,
                                           face_filename)
        else:
            body_model_path = hf_hub_download(pretrained_model_or_path,
                                              filename,
                                              cache_dir=cache_dir)
            hand_model_path = hf_hub_download(pretrained_model_or_path,
                                              hand_filename,
                                              cache_dir=cache_dir)
            face_model_path = hf_hub_download(face_pretrained_model_or_path,
                                              face_filename,
                                              cache_dir=cache_dir)
    ...
    def detect_hands(
            self, body: BodyResult,
            oriImg) -> Tuple[Union[HandResult, None], Union[HandResult, None]]:
        left_hand = None
        right_hand = None
        H, W, _ = oriImg.shape
        for x, y, w, is_left in util.handDetect(body, oriImg):
            peaks = self.hand_estimation(oriImg[y:y + w,
            ...
            if peaks.ndim == 2 and peaks.shape[1] == 2:
                ...
                hand_result = [
                    Keypoint(x=peak[0], y=peak[1]) for peak in peaks
                ]
    ...
    def detect_poses(self,
                     oriImg,
                     include_hand=False,
                     include_face=False) -> List[PoseResult]:
        """
        ...
        with torch.no_grad():
            ...
            for body in bodies:
                ...
                results.append(
                    PoseResult(
                        BodyResult(keypoints=[
                            Keypoint(x=keypoint.x / float(W),
                                     y=keypoint.y / float(H))
                            if keypoint is not None else None
                            for keypoint in body.keypoints
                        ],
                                   total_score=body.total_score,
                                   total_parts=body.total_parts), left_hand,
                        right_hand, face))
    ...
    def __call__(self,
        ...
        if "return_pil" in kwargs:
            warnings.warn("return_pil is deprecated. Use output_type instead.",
                          DeprecationWarning)
